...
Ping = vape.Legit:CreateModule({
	...
	Function = function(callback)
		if callback then
			repeat
				label.Text = math.floor(tonumber(cloneref(game:GetService('Stats')):FindFirstChild('PerformanceStats').Ping:GetValue()))..' ms' -- bad practise xylex.. don't underestimate me >:(
				task.wait(1)
